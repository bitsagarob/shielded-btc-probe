...
use shielded_probe::chain::{Electrum, DEFAULT_ELECTRUM};
use shielded_probe::envelope::Payout;
use shielded_probe::indexer::{Deployment, Event, State};
use shielded_probe::prover::Params;
use shielded_probe::wallet::{parse_address, Wallet};
use std::path::{Path, PathBuf};
use std::time::Instant;

#[derive(Parser)]
#[command(name = "sbp", about = "Shielded Bitcoin transfer-layer probe on Bitsaga Signet")]
struct Cli {
...
fn load_state(dir: &Path) -> Result<State> {
    let dep: Deployment = serde_json::from_reader(std::fs::File::open(dir.join("deployment.json"))?)?;
...
fn synced_state(cli: &Cli, params: &Params) -> Result<(State, Electrum)> {
    ...
    ensure!(st.deployment.vk_fingerprint == params.vk_fingerprint(), "verifying key does not match the deployment");
    let t = Instant::now();
    let n = st.sync(&mut e, params)?;
    st.save(&cli.state.join("state.json"))?;
    if n > 0 {
        eprintln!("replayed {n} blocks in {:.1}s, now at {}", t.elapsed().as_secs_f64(), st.replayed_height);
    }
    Ok((st, e))
}
...
fn main() -> Result<()> {
    ...
    match &cli.cmd {
        Cmd::Bench => {
            ...
            use shielded_probe::circuit::{sample::sample_witness, TransferCircuit};
            let (w, p) = sample_witness();
            let cs = ConstraintSystem::<shielded_probe::Fr>::new_ref();
            TransferCircuit { public: Some(p.clone()), witness: Some(w.clone()) }.generate_constraints(cs.clone())?;
            ...
            for _ in 0..3 {
                ...
                println!("prove {ps:.2}s  verify {:.1}ms  ok={ok}  proof {} bytes", t.elapsed().as_secs_f64() * 1000.0, proof.len());
            }
            println!("threads            {}", std::thread::available_parallelism().map(|n| n.get()).unwrap_or(1));
        }
        Cmd::Deploy { activation, operator_wallet } => {
        ...
        Cmd::Scan { wallet } => {
            ...
            for note in &w.file.notes {
                println!(
                    ...
                    if note.locked_by.is_some() { " (locked)" } else { "" },
                    note.source_txid
                );
            }
        ...
        Cmd::Send { wallet, to, amount } => {
            ...
            println!("send txid {txid}\nenvelope {bytes} bytes, carrier {vsize} vB, proof {prove_s:.2}s, anchor {}", st.replayed_height);
        }
        Cmd::Redeem { wallet, amount, to } => {
            ...
            let addr: bitcoin::Address = to.parse::<bitcoin::Address<_>>()?.require_network(shielded_probe::chain::NETWORK)?;
...
fn print_status(st: &State) {
    ...
    for ev in &st.events {
        match ev {
            Event::Mint(m) => println!("  block {} mint {} sat -> pos {}  {}", m.height, m.value, m.pos, m.txid),
            Event::Transfer(t) => println!("  block {} transfer -> pos {:?}  {}", t.height, t.positions, t.txid),
        }
    }
